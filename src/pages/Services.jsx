      ...
      <div className="p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col">
          ...
          <button onClick={() => setOpen(o => !o)}
            className="w-full flex items-center justify-between text-brand-orange font-semibold text-sm py-3 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-4">
                    ...
                    <ul className="flex flex-col gap-2">
                      {s.process.map((p, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-slate-600">
                          <span className="w-5 h-5 rounded-full bg-brand-orange/12 text-brand-orange font-bold text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                          {p}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    ...
                    <ul className="flex flex-col gap-2">
                      {s.documents.map((d, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm text-slate-600">
                          <span className="text-brand-green-light font-bold mt-0.5">✓</span>{d}
                        </li>
                      ))}
                    </ul>
                  </div>
...
export default function Services() {
  return (
    <>
      {/* Hero */}      <section className="relative pt-32 pb-16 bg-navy overflow-hidden">
        <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.03) 1px,transparent 1px)', backgroundSize: '60px 60px' }} />
        <motion.div className="absolute w-[500px] h-[500px] rounded-full opacity-15 blur-[80px]"
          style={{ background: 'radial-gradient(circle,var(--color-brand-orange),transparent)', top: '-10%', right: '0' }}
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}
            ...
            Complete NPA &amp; Loan<br /><span className="text-brand-orange">Recovery Services</span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            ...
            {services.map(s => (
              <a key={s.id} href={`#${s.id}`}
                className="text-xs font-semibold bg-white/8 border border-white/15 text-white/80 px-3 py-1.5 rounded-full hover:bg-brand-orange hover:text-white hover:border-brand-orange transition-all">
                {s.icon} {s.title}
              </a>
            ))}
          </motion.div>
