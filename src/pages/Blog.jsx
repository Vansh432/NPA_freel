    ...
    <>
      ...
      <section className="relative pt-32 pb-16 bg-navy overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="inline-flex items-center gap-1.5 bg-gold/15 text-gold border border-gold/30 text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-6">
            Blog & Resources
          </motion.span>
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="font-serif text-5xl md:text-6xl text-white mb-5">
            NPA & Banking<br /><span className="text-gold">Knowledge Hub</span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-2 mb-10">
            {categories.map(c => (
              <button key={c} onClick={() => setActive(c)}
                className={`text-sm font-semibold px-4 py-2 rounded-full transition-all ${active === c ? 'bg-navy text-white' : 'bg-white border border-slate-200 text-slate-600 hover:border-gold hover:text-gold-dark'}`}>
                {c}
              </button>
            ))}
          </motion.div>
            ...
            {filtered.map((b, i) => (
              <motion.div key={b.id}
                ...
                <div className="p-7 flex flex-col h-full">
                  <span className="text-xs font-bold text-gold-dark bg-gold/10 px-3 py-1 rounded-full mb-3 self-start">{b.category}</span>
      ...
      <section className="relative pt-32 pb-16 bg-navy overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-6 relative z-10">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <Link to="/blog" className="inline-flex items-center gap-1 text-white/60 hover:text-gold text-sm mb-6 transition-colors">
              <ArrowLeft size={16} /> Back to Blog
            </Link>
          </motion.div>
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
            className="inline-flex items-center text-xs font-bold text-gold-dark bg-gold/15 border border-gold/30 px-3 py-1 rounded-full mb-4">
            {post.category}
          </motion.span>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            className="text-white/50 text-sm">{post.date} · NPA Bazar Editorial</motion.p>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {blogs.filter(b => b.id !== id).slice(0, 2).map(b => (
                <Link key={b.id} to={`/blog/${b.id}`}
                  className="bg-slate-50 border border-slate-200 rounded-xl p-5 hover:border-gold hover:shadow-lg transition-all group">
                  <span className="text-xs font-bold text-gold-dark">{b.category}</span>
