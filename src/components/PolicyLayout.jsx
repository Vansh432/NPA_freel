    ...
    <>
      ...
      <section className="relative pt-32 pb-20 bg-navy overflow-hidden">
        ...
        <motion.div
          ...
          style={{ background: 'radial-gradient(circle,#d4a853,transparent)', top: '-20%', right: '-5%' }}
          animate={{ scale: [1, 1.15, 1] }}
          transition={{ duration: 6, repeat: Infinity }}
        />
        <motion.div
          className="absolute w-[380px] h-[380px] rounded-full opacity-10 blur-[80px]"
          style={{ background: 'radial-gradient(circle,#3b6fd4,transparent)', bottom: '-15%', left: '-5%' }}
                ...
                <ul className="flex flex-col gap-1.5 max-h-[70vh] overflow-y-auto pr-1">
                  {sections.map((s) => (
                    <li key={s.id}>
                      <a
                        href={`#${s.id}`}
                        className="block text-sm text-slate-500 hover:text-gold-dark hover:bg-white border-l-2 border-transparent hover:border-gold transition-all pl-3 py-1.5 rounded-r-md"
                      >
                        {s.heading}
                      </a>
                    </li>
              ...
              <div className="flex flex-col gap-5">
                {sections.map((s, i) => (
                  <motion.article
                    ...
                    className="bg-white border border-slate-200 rounded-2xl p-7 scroll-mt-28 hover:border-gold/40 hover:shadow-md transition-all"
                  >
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6">
          <motion.div
            {...fade()}
            className="relative rounded-3xl overflow-hidden bg-navy text-white p-10 md:p-12"
          >
            <motion.div
              className="absolute w-[400px] h-[400px] rounded-full opacity-15 blur-[80px] pointer-events-none"
              style={{ background: 'radial-gradient(circle,#d4a853,transparent)', top: '-30%', right: '-10%' }}
              animate={{ scale: [1, 1.15, 1] }}
              transition={{ duration: 7, repeat: Infinity }}
            />
            <div className="relative z-10">
              <span className="inline-flex items-center gap-2 bg-gold/15 text-gold border border-gold/30 text-xs font-bold uppercase tracking-widest px-3.5 py-1.5 rounded-full mb-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                <a
                  ...
                  className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3.5 hover:bg-white/10 hover:border-gold/40 transition-all"
                >
              ...
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 bg-gold text-navy font-bold px-6 py-3 rounded-xl hover:-translate-y-0.5 hover:shadow-lg hover:shadow-gold/30 transition-all"
              >
